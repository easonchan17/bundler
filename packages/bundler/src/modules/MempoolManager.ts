...
export class MempoolManager {
  ...
  removeUserOp (userOpOrHash: UserOperation | string): void {
    ...
    if (index !== -1) {
      ...
      const timestamp: number = new Date().getTime();
      console.log('#MempoolManager: removeUserOp timestamp=', timestamp, 'mempool length=', this.mempool.length, 'userOp nonce=', userOp.nonce, 'sender=', userOp.sender)
  ...
  dump (): MempoolDump {
    const timestamp: number = new Date().getTime();
    console.log('#MempoolDump timestamp=', timestamp, this.mempool.length )
